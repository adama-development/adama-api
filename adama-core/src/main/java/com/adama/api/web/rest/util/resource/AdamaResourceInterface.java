...
public interface AdamaResourceInterface<T extends DeleteEntityAbstract, D extends AdamaDtoAbstract> {
	 ...
	 * @param request
	 * @return the ResponseEntity with status 201 (Created) and with body the new entityDTO, or with status 400 (Bad Request) if the entity has
	 ...
	 * @param request
	 * @return the ResponseEntity with status 200 (OK) and with body the updated entityDTO, or with status 400 (Bad Request) if the entityDTO is not
	 *         valid, or with status 500 (Internal Server Error) if the entityDTO couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Entity updated return in body"),
			@ApiResponse(code = 400, message = "The Entity is not valid"), @ApiResponse(code = 500, message = " The Entity couldnt be updated") })
	ResponseEntity<D> updateEntity(D dto, HttpServletRequest request) throws URISyntaxException;

	/**
	 * GET /entities : get all the Entities. Can paginate with page=1&size=20. Sorting with sort='field'.ASC. Get Excel by Header Accept as
	...
	ResponseEntity<?> getAllEntities(String search, Boolean all, Pageable pageable, HttpServletRequest request) throws URISyntaxException,
			ExcelException;;
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the EntityDTO, or with status 404 (Not Found)
	 */
